...
COPY src/sim_test_harness/package.xml ./src/sim_test_harness/package.xml
...
COPY src/sim_test_harness ./src/sim_test_harness
...
RUN bash -c "source /opt/ros/jazzy/setup.bash && \
    source install/setup.bash && \
    colcon build \
    --packages-select stonefish_ros2 controller_stonefish detection_mocker bringup sim_test_harness \
